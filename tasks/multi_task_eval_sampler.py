...
from tasks.abstract_task_sampler import (
    AbstractVIDATaskSampler,
)

from tasks.object_nav_task_sampler import ObjectNavTaskSampler
...
class MultiTaskSampler(AbstractVIDATaskSampler):
    def __init__(
        self,
    ...
    def current_house_index(self) -> int:
        ...
        # TODO: Remove "house_number" once we have moved to new datasets
    ...
    def task_spec_to_task_info(
        ...
        task_info = {
            ...
            "agent_masks": task_spec.get("agent_masks", {}),
            # "agent_camera_params": json.loads(task_spec["eval_info"]["agent_camera_params"]),
            # "agent_camera_params": json.loads(task_spec["eval_info"]["agent_camera_params"]),
        }

        if "agent_camera_params" in task_spec:
            task_info["agent_camera_params"] = task_spec["agent_camera_params"]
            
        if "eval_info" in task_spec:
            task_info["eval_info"] = task_spec["eval_info"]
            if "agent_camera_params" in task_spec["eval_info"]:
                task_info["agent_camera_params"] = json.loads(
                    task_spec["eval_info"]["agent_camera_params"]
                )
