...
from utils.metric_utils import sc_metric
...
class AbstractVIDATask(Task["StretchController"]):
    ...
    def __init__(
        ...
        max_steps: int,
        action_space: Type[AbstractActionSpace] = None,  # SPOCV1ActionSpace(),
        reward_config: Optional[RewardConfig] = None,
        house: Optional[Dict[str, Any]] = None,
        collect_observations: bool = True,
        task_sampler: Optional["AbstractVIDATaskSampler"] = None,
        **kwargs,
    ) -> None:
    ...
    def record_observations(self):
        ...
        # TODO KIANA VERY IMPORTANT WHY IS THIS AN ISSUE, HAS TO BE UNCOMMENTED
        # if len(self.observation_history) == 1 and self.num_steps_taken() == 0:
        #     self.observation_history = []
    ...
    # TODO: how to translate action space that can be heterogeneous and complicated within the gym framework? necessary?
    @property
    def action_space(self):
        # KHZ: Use the input action_space for now
    ...
    def step_with_action_str(self, action_name: str, is_random=False):
        assert isinstance(
            self.agent_action_space, SPOCV1ActionSpace
        ), "Only SPOC action space is supported for raw strings."
        assert (
            action_name in self.agent_action_space.long_action_names
        ), f"Action {action_name} not in action space."
        self.last_action_random = is_random

        return self.step(action_name)

    # TODO: hate this lots, fix it later, maybe something like planner_step
    ...
    def get_action_from_goal(
        ...
        if absolute_goal in [DONE_ACTION, SUB_DONE_ACTION, PICKUP_ACTION, DROPOFF_ACTION]:
            ...
            assert (
                agent_relative_goal is not None
            ), "Agent relative goal must be provided for spatial action."
            assert current_state is not None, "Current state must be provided for spatial action."
    ...
    def step(self, action: Any) -> RLStepResult:
        ...
        # TODO: Here is where additional parsing will need to be added to handle what the model outputs mechanically
        # Right now this will work with step_with_action_str and the SPOC action space (ish)
        # Need to turn some continuous token(s) into something that will work with Continuous Action Space action calls
        # for now wants an action_string and an action_quantity

        if isinstance(action, int):
            # KHZ: Only SPOCV1ActionSpace supports get_action_list function
    ...
    def _step(self, action: StretchAction) -> RLStepResult:
        ...
        self.last_action_time_estimate = copy.deepcopy(self.last_action_time_estimate_temp)
