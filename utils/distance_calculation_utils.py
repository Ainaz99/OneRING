...
def min_l2_distance_and_target_point(source, target_points):
    return min(
        [(position_dist(target, source), target) for target in target_points], key=lambda x: x[0]
    )
...
def closest_reachable_point_and_distance(controller: "StretchController", reachable_points):
...
def get_approximate_shortest_path_to_room(
    ...
    if points_in_room is None:
        ...
        points_in_room = reachable_points_in_room(reachable_points, room_polymap[room_id])
