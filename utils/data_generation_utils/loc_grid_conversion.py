...
def locs2grids(
    ...
    if not use_mesh_grid:
        ...
        locs_grid[:, :, 0] = locs_grid[:, :, 0].sum(axis=1, keepdims=True) / valid_grid.sum(
            axis=1, keepdims=True
        )
        locs_grid[:, :, 1] = locs_grid[:, :, 1].sum() / valid_grid.sum()
        locs_grid[:, :, 2] = locs_grid[:, :, 2].sum(axis=0, keepdims=True) / valid_grid.sum(
        ...
        row_centroids = np.concatenate([bins_x - grid_spacing / 2, bins_x[-1:] + grid_spacing / 2])
        col_centroids = np.concatenate([bins_z - grid_spacing / 2, bins_z[-1:] + grid_spacing / 2])
...
def upscale_grids(valid_grid, locs_grid, digitizer, factor=3, optimistic=False):
    ...
    # assume constant y
    y_grid = np.ones(valid_grid.shape, dtype=locs_grid.dtype) * np.max(locs_grid[:, :, 1])
