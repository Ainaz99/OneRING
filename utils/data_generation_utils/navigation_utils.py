...
from utils.data_generation_utils.exception_utils import PlannerFailedToSeeEnoughPixelsException

if TYPE_CHECKING:
    from tasks import AbstractVIDATask

from utils.constants.stretch_initialization_utils import GRID_SIZE
from utils.data_generation_utils.loc_grid_conversion import locs2grids, grids2locs
from utils.distance_calculation_utils import position_dist, sum_dist_path, all_distances

from utils.data_generation_utils.bbox_utils import get_box_from_object, get_basis_for_3d_box
from utils.constants.stretch_initialization_utils import STRETCH_ENV_ARGS
from environment.actions import StretchAction
from environment.action_spaces import DONE_ACTION
from environment.stretch_state import StretchState, convert_relative_to_world_coordinate
...
TEMP_VERBOSE = False  # KE: Not sure who is in the process of deugging this but for now I have made a flag for all the prints. Please remove all the verbose once done with debugging.
...
def default_stop_callback(task: "AbstractVIDATask") -> bool:
    return False


def action_towards_goal_dict(task: "AbstractVIDATask", goal_dict: Dict) -> Optional[StretchAction]:
...
def achieve_goal_dict_with_action_space(task: "AbstractVIDATask", goal_dict: Dict) -> bool:
    ...
    while action_success:
        ...
        if action is None:
            break
        event = task.step_with_random(action, is_random=False)
...
def action_space_can_act_on_path(task, path):
    ...
    if path is None or len(path) == 0:
        return False
    return action_towards_goal_dict(task, goal_dict={"base_position": path[-1]}) is not None
...
def walk_on_path(
    ...
    successful_stop_callback: Callable[["AbstractVIDATask"], bool] = default_stop_callback,
    replan_func=default_nav_replan_func,
    return_success: bool = True,
):
    if stop_for is not None:
        stop_for = set(stop_for)
        combined_successful_stop_callback = (
            lambda t: successful_stop_callback(t)
    ...
    while len(path or []) > 0:
        if total_failures_to_follow_path > max_tries:
            ...
            # raise PlannerFailedMaxSteps("Failed to follow path after max tries")
            break
        ...
        if not move_success:
            ...
            def base_step_direction_random_quantity(direction: str):
                if direction == "back":
                    action_quantity = task.agent_action_space.sample_random_action_quantity(
                    ...
                    action_quantity = -action_quantity if direction == "left" else action_quantity
                    world_theta = (
                        StretchState(task.controller).base_position["theta"] + action_quantity
                    )
            ...
            undo_sr = undo_failed_action(action)
            ...
            if not move_success:
                ...
                for _ in range(5):
                    ...
                    random_action_success = base_step_direction_random_quantity(random_direction)
                    move_success = task.controller.sufficient_agent_state_change(
                        lock_state, StretchState(task.controller)
                    )
                    if move_success:
...
def rotate_until_visible(
    ...
    say_done: bool = False,
    step_callback: Callable[["AbstractVIDATask"], bool] = default_stop_callback,
    use_wall_alignment: bool = False,
    manipulation_camera: bool = False,
):
    assert not (use_task_success and manipulation_camera)
    assert not (
        use_task_success and use_object_nav_strict_success
    ...
    for i in range(20):  # arbitrary number of rotations. TODO revisit
        ...
        goal_delta_state, goal_abs_state = StretchState.create_delta_from_goal(
            current_state=current_state,
            goal_dict={"base_position": {"theta": vector + current_state.base_position["theta"]}},
        )
...
def rotate_until_visible_and_say_done(task, object_id, step_callback=default_stop_callback):
    return rotate_until_visible(
        task, object_id, use_task_success=True, say_done=True, step_callback=step_callback
    )
...
def build_layout_graph_matrix(house, room_polymap):
    ...
    # P2P change: modify doors to work even if room1 is none (i.e. exterior door, which is skipped)
    doors = [sorted([d["room0"], d["room1"]]) for d in house["doors"] if d["room1"] is not None]
...
def filtered_starting_positions(locations, margin=1, grid_spacing=0.25) -> List[Dict[str, float]]:
...
def visit_ids_from_info(
    ...
    while len(all_targets):
        current_target_id = task.controller.get_closest_object_from_ids(list(all_targets))
        ...
        while len(left_to_visit) + len(failures) > 0:
            ...
            if len(failures) > 0 and (len(left_to_visit) == 0 or random.random() > 0.5):
                closest_object_id = task.controller.get_closest_object_from_ids(list(failures))

            if closest_object_id is None:
                closest_object_id = task.controller.get_closest_object_from_ids(list(left_to_visit))
            ...
            if use_astar:
                ...
                path = task.controller.get_shortest_path_to_object(object_id=closest_object_id)
            if path is not None:
                nav_succ = walk_on_path(
                    path,
                    task,
                    max_tries=20,
...
def get_nearest_to_wall_center_in_room(task, wall_id, room_id, thinned_starting_positions):
    ...
    positions.sort(key=lambda p: sum((p[k] - world_position[k]) ** 2 for k in ["x", "z"]))
...
def get_object_room_id(obj, room_poly_map, verbose=False):
    ...
    return max([(count, room_id) for room_id, count in room_ids.items()], key=lambda x: x[0])[1]


def object_ids_in_closed_receptacles(controller: "StretchController", closed_openness=0.1):
...
def front_facing_positions(
    ...
    all_dists = np.min(all_distances(reachable_positions, seed_positions, ignore_y=True), axis=1)
...
def get_room_to_walls(house, min_wall_width: Optional[float] = None):
    ...
    def get_wall_width_start(wall_id):
        wall_xzs = wall_id.split("|")[2:]
        # P2P update: if any of the walls cannot convert to float, split on _ and take the first one
        wall_xzs = [x.split("_")[0] for x in wall_xzs]
        assert len(wall_xzs) == 4
        difx = abs(float(wall_xzs[2]) - float(wall_xzs[0]))
        difz = abs(float(wall_xzs[3]) - float(wall_xzs[1]))

        if difx >= difz:
            return LineString([(0, 0), (difx, 0)]), min(float(wall_xzs[2]), float(wall_xzs[0]))
        else:
            return LineString([(0, 0), (difz, 0)]), min(float(wall_xzs[3]), float(wall_xzs[1]))
...
def is_any_object_sufficiently_visible_and_in_center_frame(
    ...
    for obj_id, obj_data in target_visibility_quant.items():
        ...
        # TODO comment only for phone2proc
        ...
        if obj_data["empty_top"] is not None and (not obj_data["empty_top"]):
            continue
        # TODO end comment for phone2proc

        sufficient_visibility = True
        break
    print(sufficient_visibility, raise_exception)
    
    #TODO AINAZ REMOVE THE FOR LOOP BELOW AFTER DOUBLE CHECKING WITH ROSE WHY BASKETBALL IS 800 pixels
    if sufficient_visibility is False:
        for obj_id in object_ids:
            segm = controller.get_segmentation_mask_of_object(obj_id,which_camera='nav')
            if segm.sum() > 300:
                sufficient_visibility = True
    
    if sufficient_visibility is False and raise_exception:
        raise PlannerFailedToSeeEnoughPixelsException(
            "Planner Failed to see the enough pixels of the object"
        )
...
class WallSearch:
    def __init__(
        ...
        self.wall_alignment_threshold = wall_alignment_threshold  # degrees (>180 == disabled)
        ...
        self.dot_prod_threshold = np.cos(np.deg2rad(180 - self.wall_alignment_threshold))
    ...
    def end_nav_on_seen_wall(self, task: "AbstractVIDATask") -> bool:
        ...
        if self.targeting_room is not None:
            ...
            for wall_id in self.room2walls[self.targeting_room] - self.seen_walls:
                wall_norm = wall_normal(wall_id, self.targeting_room, task.room_poly_map)
    ...
    def seen_room_walls_in_step(self, task: "AbstractVIDATask") -> bool:
    ...
    def end_nav_when_entering_room(self, task: "AbstractVIDATask") -> bool:
...
def wall_normal(wall_id, room_id, room_polymap, shift=0.1):
    ...
    along = np.array(
        [float(wall_xzs[2]) - float(wall_xzs[0]), 0, float(wall_xzs[3]) - float(wall_xzs[1])]
    )
    ...
    if get_room_id_from_location({room_id: room_polymap[room_id]}, front, verbose=False) == room_id:
        return ortho
