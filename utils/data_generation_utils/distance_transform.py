...
def nearest_safest_discrete_location_pose_and_dist(poses, dt, digitizer, object_id, planner):
...
def most_central_discrete_location_pose_and_dist(poses, digitizer):
    ...
    upfactor = 3
    imup = cv2.resize(
        im.astype(np.uint8), None, fx=upfactor, fy=upfactor, interpolation=cv2.INTER_NEAREST
    )
