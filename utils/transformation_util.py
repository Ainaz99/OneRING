...
def get_full_transformation_list(size, version="v2"):
    if version == "v2":
        ...
        raise NotImplementedError(f"data augmentation versions supported are v1 and v2, got {name}")
...
def test_apply_same_transformation():
    ...
    for i in range(100):
        controller.controller.step(random.choice(action_list))
        frame = controller.controller.last_event.distortion_frame[:, :, :3]
        frames.append(frame)
        frame_tensorized = (
            torch.Tensor(frame.copy()).unsqueeze(0).permute(0, 3, 1, 2).float() / 255.0
        )
        augmented_frame = transformation(frame_tensorized)
        augmented_frame = (
            augmented_frame.permute(0, 2, 3, 1).squeeze(0).cpu().numpy() * 255
        ).astype(np.uint8)
        augmented_frames.append(augmented_frame)
        combined_frame = np.concatenate([frame, augmented_frame], axis=1)
...
if __name__ == "__main__":
    random.seed(10)
    test_apply_same_transformation()
