...
# Excluded hypernyms were created by running get_hypernym_to_descendants_for_all_vida_synsets()
...
OLD_EXCLUDED_HYPERNYMS = frozenset(
    {
        "entity.n.01",
...
def is_subsynset_of(synset: Union[str, Synset], other_synset: Union[str, Synset]) -> bool:
    return is_hypernym_of(synset=synset, possible_hypernym=other_synset)


def symmetric_subsynset_of(synset: Union[str, Synset], other_synset: Union[str, Synset]) -> bool:
...
def filter_synsets_to_remove_hyponyms(synsets: Union[Sequence[str], Sequence[Synset]]) -> List[str]:
    if len(synsets) == 0:
        return []
...
def get_all_synsets_in_vida() -> List[Synset]:
...
def get_hypernym_to_descendants_for_all_vida_synsets():
    synsets = get_all_synsets_in_vida()
...
if __name__ == "__main__":
    ...
    def ensure_valid_hypernyms():
        old_anns = get_objaverse_annotations(OBJAVERSE_HOME_COMMIT_ID)
        new_anns = get_objaverse_annotations()
        assert len(old_anns) < len(new_anns), f"{len(old_anns)} old anns >= {len(new_anns)} new ans"

        synsets = set(ann["synset"] for ann in old_anns.values()) | set(
            AI2THOR_OBJECT_TYPE_TO_WORDNET_SYNSET.values()
        )
        synsets = set([wn.synset(s) for s in synsets])
        synsets = [s.name() for s in synsets]
        ...
        good_old_hypernyms = set(hypernym_to_descendants.keys()) - OLD_EXCLUDED_HYPERNYMS

        new_excluded_hypernyms = (EXCLUDED_HYPERNYMS | OLD_EXCLUDED_HYPERNYMS) - good_old_hypernyms

        print(sorted(list(new_excluded_hypernyms)))

        print(
            f"{len(EXCLUDED_HYPERNYMS)} current exclusions {len(new_excluded_hypernyms)} when keeping good ones and {len(OLD_EXCLUDED_HYPERNYMS)} old exclusions"
        )

        print(f"Re-allowed {sorted(list(EXCLUDED_HYPERNYMS - new_excluded_hypernyms))}")

    ensure_valid_hypernyms()
    print("DONE")
