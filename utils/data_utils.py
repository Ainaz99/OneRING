    ...
    try:
        ...
        with h5py.File(path, "r") as d:
            for k in d.keys():
                ...
                if "agent_parameter_sensor" in d[k].keys():
                    j["agent_parameter_sensor"] = json_templated_spec_to_dict(convert_byte_to_string(d[k]["agent_parameter_sensor"][0, :]))
...
class Hdf5TaskSpecs:
    ...
    def __init__(
        ...
        if data is None:
            ...
            print(f"Loading hdf5_sensors.hdf5 files from {self.subset_dir}----------")
        ...
        print(f"Loaded {len(self.data)} task specs")
        ...
        print(f"Using {len(self.data)} task specs")
    ...
    def __add__(self, other: "Hdf5TaskSpecs") -> "Hdf5TaskSpecs":
        """Combine two datasets into a new dataset."""
        if not isinstance(other, Hdf5TaskSpecs):
            raise TypeError(f"Cannot add Hdf5TaskSpecs with {type(other)}")

        # Combine the data from both datasets
        combined_data = self.data + other.data

        # Sort the combined data by the 'house_index' key
        combined_data.sort(key=lambda x: x.get("house_index"))

        # Create a new instance with the combined data
        return Hdf5TaskSpecs(
            subset_dir=self.subset_dir,  # The subset_dir can be arbitrary; it doesn't affect combined data.
            data=combined_data,
            proc_id=self.proc_id,
            total_procs=self.total_procs,
        )
