...
from utils.constants.stretch_initialization_utils import AGENT_MODE, AGENTS_BASE_HEIGHT
...
class StretchState:
    ...
    agent_center_y_height = (
        AGENTS_BASE_HEIGHT  # prefer this to be a constant. Not fixated on it though.
    )
    ...
    def __init__(self, controller=None):
        if controller is not None:
            ...
            if AGENT_MODE != "fpin":
                ...
                held_oids = set(controller.last_event.metadata["arm"].get("heldObjects", []))
                self._wrist_pose = {
                    "y": wrist_pose["y"],
                    "z": wrist_pose["z"],
                    "yaw": wrist_pose["yaw"],
                ...
                # Dummy values cause the agent doesn't have an arm
    ...
    def get_wrist_pose(controller):
        ...
        arm_metadata = controller.last_event.metadata["arm"]
        if "joints" not in arm_metadata:
            # this is a real robot, not a simulation
            # TODO: Align these values together.
            return {
                "y": arm_metadata["lift_m"],
                "z": arm_metadata["extension_m"],
                "yaw": arm_metadata["wrist_degrees"],
            }
    ...
    def get_absolute_hand_position(controller):
        if "joints" not in controller.last_event.metadata["arm"]:
            # this is a real robot, not a simulation
            return {"x": None, "y": None, "z": 0}
...
def calc_inverse_full_rotation(full_rotation):
    rotation = R.from_euler(
        "xyz", [full_rotation["x"], full_rotation["y"], full_rotation["z"]], degrees=True
    )
...
def convert_world_to_relative_coordinate(world_obj, relative_location, relative_rotation):
    ...
    if "theta" in relative_rotation:
        # KIANA CAN"T REMOVE but important?
        # assert 'x' not in relative_rotation and 'y' not in relative_rotation and 'z' not in relative_rotation
        inverse_agent_rotation = calc_inverse_only_y(relative_rotation["theta"])
    elif "x" in relative_rotation and "y" in relative_rotation and "z" in relative_rotation:
        inverse_agent_rotation = calc_inverse_full_rotation(relative_rotation)
    else:
        raise ValueError("relative_rotation must have either 'theta' or 'x', 'y', 'z' keys")
...
def convert_world_to_relative_coordinate_batched(world_obj, relative_location_rotation):
    ...
    if "theta" in relative_location_rotation:
        inverse_agent_rotation = calc_inverse_only_y(relative_location_rotation["theta"])
    else:
        assert (
        ...
        inverse_agent_rotation = calc_inverse_full_rotation(relative_location_rotation["rotation"])

    # Note that we will be ignoring thetas for final location
    if type(world_obj) == list and len(world_obj) > 0 and type(world_obj[0]) == dict:
        world_positions = np.array([[obj["x"], obj["y"], obj["z"]] for obj in world_obj])
    else:
        assert type(world_obj) == np.ndarray and world_obj.shape[1] == 3
