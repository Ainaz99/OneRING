...
from environment.stretch_state import (
    StretchState,
...
# DO NOT ADD IMPORTS. VERY HIGH RISK OF CIRCULARITY.

# These are special action strings that will exist in any action space, and it is useful to have them defined here.
# Not married to this location, but they should be somehow treated specially & it's a reasonable place to start.
DONE_ACTION = "done"
...
### Helper functions for state difference actions ###


# TODO: there's a nice way to combine these two functions, surely. Leave it for now.
# n.b. these need to be here or in stretch state because they are used in the action space
# and this can't have vida imports.


def load_action_space_from_disk(hdf5_path: str = None, direct_path: str = None):
    criteria = [hdf5_path is not None, direct_path is not None]
    assert (
        sum(criteria) == 1
    ), f"Exactly one of hdf5_path or direct_path must be provided. Got {criteria}"
    if hdf5_path is not None:
        # EG
        file_to_load = "/".join(hdf5_path.split("/")[:-3]) + "/action_space.pkl"
    elif direct_path is not None:
        file_to_load = direct_path
    else:
        raise ValueError("Exactly one of hdf5_path or direct_path must be provided.")
    try:
        with open(file_to_load, "rb") as f:
            action_space = pickle.load(f)
            return action_space
    except Exception:
        print(f"Could not load action space from {file_to_load}, SWITCHING TO DEFAULT")
        return SPOCV1ActionSpace()  # KE: This can turn into a raise exception later
...
class AbstractActionSpace(abc.ABC):
    ...
    @abc.abstractmethod
    def _get_singleton_action(self, action_string: str = "", action_quantity: float = None):
    ...
    # simple for the SPOC V1 but requires more thought otherwise
    ...
    @staticmethod
    def _set_action_space_tolerance(base_meters=0.05, arm_meters=0.02, degrees=3) -> StretchState:
    ...
    def get_action_from_meaningful_action_string(self, action_string) -> AbstractAction:
        if action_string in [PICKUP_ACTION, DROPOFF_ACTION, DONE_ACTION, SUB_DONE_ACTION]:
    ...
    def sample_random_action_quantity(self, action_string: str):
        if action_string in ["base_z", "arm_y", "arm_z"]:
            return random.uniform(self.random_min_translation, self.random_max_translation)
...
class SPOCV1ActionSpace(AbstractActionSpace):
    def __init__(self):
        ...
        self.random_min_translation = 0.2 + self.random_translation_eps  # larger than tolerance
        ...
        self.base_actions = [
            self.action_type(
                base=dict(z=self.base_move_constant), long_name="move_ahead", short_name="m"
            ),
            self.action_type(
                base=dict(z=-self.base_move_constant), long_name="move_back", short_name="b"
            ),
            self.action_type(
                base=dict(theta=-self.base_rot_constant), long_name="rotate_left", short_name="l"
            ),
            self.action_type(
                base=dict(theta=self.base_rot_constant), long_name="rotate_right", short_name="r"
            ),
            self.action_type(
        ...
        self.arm_actions = [
            self.action_type(
                arm=dict(y=self.arm_move_constant), long_name="move_arm_up", short_name="yp"
            ),
            self.action_type(
                arm=dict(y=-self.arm_move_constant), long_name="move_arm_down", short_name="ym"
            ),
            self.action_type(
                arm=dict(z=self.arm_move_constant), long_name="move_arm_out", short_name="zp"
            ),
            self.action_type(
                arm=dict(z=-self.arm_move_constant), long_name="move_arm_in", short_name="zm"
            ),
            self.action_type(
        ...
        self.full_action_list = [
            self.get_action(short_name).long_name for short_name in self.SPOC_action_list_order
        ]
    ...
    def get_action_index_from_string(self, action_string: str):
        ...
        if action_string in self.full_action_list:
            return self.full_action_list.index(action_string)
        elif action_string in self.SPOC_action_list_order:
            return self.SPOC_action_list_order.index(action_string)
        else:
    ...
    def _get_singleton_action(self, action_string: str = "", action_quantity: float = None):
        # TODO: probably need to think about the special actions some more
        # also the names spatial/special are too close
    ...
    def calculate_time_step_needed_to_complete(self, action):
        # TODO RMH TO KE: revisit how this works in BPE to reflect actual time - see equivalent function below
        # Additional TODO: strong opinion that any tokenizer-aware logic should be kept out of the generic action spaces
        # revisit post-ICRA
    ...
    def state_difference_to_action(
        ...
        if state_difference is not None and final_state is not None:
            raise ValueError("Only one of state_difference or final_state can be provided")
        ...
        if len(relevant_params["base_position"]) > 0:
            ...
            if abs(base_rotation) > self.action_space_tolerance.base_position["theta"] * 2:
            ...
            if "x" in relevant_params["base_position"] or "z" in relevant_params["base_position"]:
        ...
        if "y" in relevant_params["wrist_pose"] and state_difference.wrist_pose["y"] > 0:
            return (
        ...
        if "z" in relevant_params["wrist_pose"] and state_difference.wrist_pose["z"] > 0:
            return (
        ...
        if len(relevant_params["held_oids"]):
            if len([oid for delta_sign, oid in state_difference.held_oids if not delta_sign]) > 0:
                return self._get_singleton_action(action_string=DROPOFF_ACTION)
            if len([oid for delta_sign, oid in state_difference.held_oids if delta_sign]) > 0:
                return self._get_singleton_action(action_string=PICKUP_ACTION)

        # If we make it here, something has gone wrong!
        # Note: At least for now in this action space, can't directly operate on delta absolute hand pose.
        raise Exception(
            "A spatial action was supposedly possible, but the action space failed to find it. Pls debug."
        )
...
def agent_alignment_to_point(
    ...
    alignment = angle_point_to_point(loc_start={"x": 0, "z": 0}, loc_goal=transformed_target)
    return alignment
