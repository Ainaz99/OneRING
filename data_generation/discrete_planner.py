...
from utils.data_generation_utils.loc_grid_conversion import grids2locs, upscale_grids, locs2grids
from utils.data_generation_utils.distance_transform import (
...
from utils.data_generation_utils.bbox_utils import main_box_diagonal, get_box_from_object
from utils.distance_calculation_utils import position_dist

from utils.data_generation_utils.exception_utils import PlannerFailedToNavigateException
...
class DiscretePlanner:
    ...
    def apply_black_list(self):
        ...
        #     plt.imshow(np.uint8(self.dist_transform), cmap='gray', vmin=0, vmax=50)
        #     plt.savefig('dist_transform.png')
        #     plt.close()
        ...
        try:
            with time_block("get reachable positions"):
                orientation = (
                    int(
                        round(
                            self.task.controller.get_current_agent_full_pose()["rotation"]["y"]
                            % 360
                        )
                    )
                    // 3
                ...
                if orientation not in self._raw_locs:
                    self._raw_locs[orientation] = self.task.controller.get_reachable_positions(
    ...
    def dist_transform(self):
        if self._dist_transform is None:
            self._dist_transform = make_distance_transform(
                self.valid_grid, self.digitizer.grid_spacing, self.distance_to_obstacle_clip
            )
        ...
        if self._graph is None:
            with time_block("create grid graph"):
                self._graph = make_grid_graph(
                    self.valid_grid, self.dist_transform, weight_exp=self.graph_weight_exponent
                )
        return self._graph
    ...
    def safest_loc_and_pose_for_oid(self, dist_transform, digitizer, object_id, self_ref):
        ...
        with time_block("filter poses"):
            ...
            half_diag = (
                main_box_diagonal(get_box_from_object(self.task.controller.get_object(object_id)))
                / 2
            )
            threshold = (
                half_diag + self.max_distance + self.low_res_spacing_scaling * self.raw_grid_size
            )
    ...
    def discrete_path_to_loc(self, oid, row, col, source_row=None, source_col=None):
        source_row_col = (
            (source_row, source_col) if source_row is not None else self.current_row_col()
        )
    ...
    def _nearest_object_id_path_dist_impl(self, object_ids, target_func):
        ...
        if safest_row_col is None:
            return best_oid, self.task.controller.get_shortest_path_to_object(best_oid), np.inf
    ...
    def path_to_loc(self, loc: Dict[str, float], from_loc=None):
        ...
        discrete_path, path_cost = self.discrete_path_to_loc(None, *target_row_col, *source_row_col)
        if discrete_path is None:
            return self.task.controller.get_shortest_path_to_point(loc)
        path = self.discrete_to_continuous_path(discrete_path)
        return path

    def blacklist_or_replan_to_loc(self, loc, get_path=True, from_loc=None, *args, **kwargs):
        if not get_path:
    ...
    def path_to_room(
        self, room_id, specific_agent_meshes=None, max_tries: int = 5, room_triangles=None
    ):
        ...
        candidate_points = [
            (loc, self.dist_transform[self.digitizer.row_col(loc)]) for loc in candidate_points
        ]
        ...
        shortest_path = self.task.controller.get_shortest_path_to_room_candidate_points(
            candidate_points, specific_agent_meshes, max_tries, use_largest_possible_mesh=True
        )
