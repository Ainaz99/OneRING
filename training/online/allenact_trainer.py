...
from training.offline.train_utils import get_latest_rl_local_ckpt_pth
...
class OnPolicyRunnerMixin(abc.ABC):
    ...
    machine_id: int = 0
    callbacks: str = "wandb_logging_callback"
    ...
    def train(
        ...
        if self.checkpoint is None:
            self.checkpoint = get_latest_rl_local_ckpt_pth(
                f"{self.output_dir}/checkpoints/{self.tag}"
            )
        self.resume_allenact_ckpt = self.checkpoint is not None
        runner.start_train(
